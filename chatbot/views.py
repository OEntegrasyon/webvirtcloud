...
instances = [name['name'] for name in list(Instance.objects.values())]
...
def stream_chat(request):
    ...
    context = "\n\n".join(retrieve_context(documents+[f"my name/user name is {request.user}.", f"an instance is a virtual machine (vm)"], prompt))+f"\n\n You are an ai assistante for WebVirtCloud and your name is ZekAI, and you are here to assist WebVirtCloud users. you should give answers just about the WebVirtCloud platform and KVM, As the user {request.user} i have this instances (vm) {instances}"
...
def llama3(prompt, context):
    ...
    for line in response.iter_lines(decode_unicode=True):
        if line:
            ...
            yield 'data: {"message": {"content": "'+dt.replace('"', "`").replace('**', "`").replace('\n', '<br>')+'"}, "done": "'+str(data["done"])+'"}\n\n'
